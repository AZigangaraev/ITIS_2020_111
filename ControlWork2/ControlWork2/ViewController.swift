...
//  ControlWork2
//
//  Created by Montypass on 10.12.2020.
//
...
class ViewController: UIViewController, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, DetailViewControllerDelegate {
    ...
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAt indexPath: IndexPath) -> CGSize {
        return CGSize(width: view.frame.width / 2 - 12, height: 120)
    }
    ...
    @IBAction func addNewCell(_ sender: Any) {
        let controller = detailViewController()
        controller.cellData = CellData(title: "Title \(cellData.count + 1)", text: "", color: .systemBlue)
        controller.delegate = self
        controller.loadViewIfNeeded()
        controller.loadData()
        navigationController?.pushViewController(controller, animated: true)
